...
//===========================================================================================================
//局部更新时候使用的的常量，它是由最近邻方法得到的一个长度,从源节点出发，每次选择一个距离最短的点来遍历所有的节点得到的路径
//每个节点都可能作为源节点来遍历
double Lnn;
//矩阵表示两两城市之间的距离
...
#pragma omp parallel for num_threads(num_thread)
    ...
    //开始核心计算
    for (int i = 0; i < NcMax; i++){//所有蚂蚁进行NcMax次模拟
        int localTour[N][2];         //局部最优路径,public,局部指这次模拟的结果，对于在这轮模拟中的每只蚂蚁来说是共享变量，是临界区
        double localBestLength = 0.0;//局部最优长度,public，同上
        double tourLength;           //当前路径长度
#pragma omp parallel for num_threads(num_thread) private(tourLength)
...
//#pragma omp critical
        ...
        if (localBestLength < globalBestLength || fabs(globalBestLength - 0.0) < 0.000001){//全局比较，并记录路径和长度
            for (int m = 0; m< N; m++){
